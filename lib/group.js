...
/**
 * Distribute a line's non-empty runs into one cell per column band (nearest
 * column wins; multiple runs landing in the same cell are space-joined in
 * reading order). Shared by the Excel grid and the Word table builders.
 *
 * @returns {Array<string|null>} one entry per column in `bounds`
 */
export function lineToCells(line, bounds) {
  const cells = new Array(bounds.length).fill(null);
  for (const it of line.items) {
    const txt = it.str.replace(/\s+/g, " ").trim();
    if (!txt) continue;
    const c = columnIndexFor(it.x, bounds);
    cells[c] = cells[c] ? `${cells[c]} ${txt}` : txt;
  }
  return cells;
}

/** Median glyph height of a line (≈ its font size in PDF points). */
export function medianHeight(line) {
  const hs = line.items.map((it) => it.h).sort((a, b) => a - b);
  return hs[Math.floor(hs.length / 2)] || 11;
}

/**
 * Most common (rounded) line height across a set of lines — i.e. the body text
 * size. Lines notably taller than this read as headings.
 */
export function bodyFontSize(lines) {
  const counts = new Map();
  for (const line of lines) {
    const h = Math.round(medianHeight(line));
...
/**
 * Heuristic: does this page read as a table? True only when there's a clear
 * multi-column structure AND most lines actually populate ≥2 columns — so a
 * page of prose with incidental alignment isn't misread as a table.
 */
export function isTabularPage(lines, bounds) {
  if (bounds.length < 2 || bounds.length > 64) return false;
  let multiCol = 0;
  for (const line of lines) {
    const filled = lineToCells(line, bounds).filter((c) => c != null).length;
    if (filled >= 2) multiCol++;
  }
  return (
    lines.length > 0 && multiCol >= Math.max(2, Math.ceil(lines.length * 0.6))
  );
}
