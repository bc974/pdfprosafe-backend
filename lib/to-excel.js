...
import { groupLines, lineToText, deriveColumns, lineToCells } from "./group.js";
...
export async function buildExcel(extracted) {
  ...
  extracted.pages.forEach((page, pageIdx) => {
    ...
    lines.forEach((line, r) => {
      ...
      // Place each item's text into its nearest column band (shared with the
      // Word table builder), then coerce numeric-looking cells to numbers.
      lineToCells(line, bounds).forEach((val, c) => {
