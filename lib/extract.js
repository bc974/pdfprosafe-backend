...
// Upper bound on pages we'll process. Beyond this the conversion is both slow
// and memory-heavy on a small instance, and almost always indicates an abuse
// case rather than a real document. Surfaced to the client as `too_many_pages`.
export const MAX_PAGES = Number(process.env.MAX_PAGES || 150);
...
export async function extractPdf(buffer) {
  ...
  if (doc.numPages > MAX_PAGES) {
    await doc.destroy();
    const e = new Error(`PDF has ${doc.numPages} pages (limit ${MAX_PAGES}).`);
